#!/bin/bash
# Ejemplo de cifrado hash con sha256sum
...
sha256sum -c archivos/Texto.txt.sha256 && echo "El archivo es el original" || echo "El archivo ha sido modificado"
...
sha256sum -c archivos/Imagen.png.sha256 && echo "El archivo es el original" || echo "El archivo ha sido modificado"
rm archivos/Imagen.png.sha256

sha512sum archivos/video.mp4 > archivos/video.mp4.sha512
sha512sum -c archivos/video.mp4.sha512 && echo "El archivo es el original" || echo "El archivo ha sido modificado"
rm archivos/video.mp4.sha512